...
لديك 18 مؤشر فني + تحليل ارتباطات وتدفق سيولة قطاعي + سياق ماكرو عالمي.

⚠️ تحذير منهجي مهم — اقرأه قبل القرار:
نموذج XGBoost الحالي مُعطّل من تأثيره على score و EV (من 2026-05-10).
السبب: AUC=0.48 (أسوأ من العشوائي)، Precision=0.42، 146 عينة فقط منحازة.
البيانات الحالية مبنية على candidates السابقة فقط (survivorship bias).
نُعيد بناء النموذج على universe كامل خلال 30 يوم القادم.

⚠️ ml_probability في البيانات قد يكون مضلّلاً — لا تعتمد عليه كعامل رئيسي.
استخدمه فقط كـ "عامل ثانوي تأكيدي" مع المؤشرات التقنية الكلاسيكية.
عاملك الأساسي: score (مبني على 18 مؤشر) + MTF + قوة القطاع + Risk/Reward.

مهمتك اليوم:
1. اختيار 5-10 أسهم مع تقييم نقدي: لماذا هذا السهم وليس غيره؟
...
6. اقتراح تعديلات أوزان بناءً على:
   ...
   - ملاحظاتك كخبير
   ⚠️ لا تعتمد على feature importance من XGBoost حالياً (النموذج غير موثوق)

═══════════════════════════════════════════
🔴 قواعد إقصاء صارمة (STRICT EXCLUSION RULES):
═══════════════════════════════════════════
هذه قواعد إقصاء قاطعة. لا تختر سهماً ينتهك أياً منها مهما كان السبب.
إذا انتهك سهم قاعدة، احذفه من picks تماماً، حتى لو بدا جذاباً.

⛔ الإقصاء 1 — EV سالب:
   إذا expected_value_pct < 0، السهم مرفوض نهائياً.
   لا اختيار "شراء" لسهم EV سالب أبداً.
   استثناء وحيد: action = "مراقبة" مسموح للـ EV قريب من الصفر (-0.5% إلى 0%).

⛔ الإقصاء 2 — Risk/Reward ضعيف:
   إذا risk_reward < 1.3، السهم مرفوض.
   ملاحظة: نُخفّض من 1.5 إلى 1.3 لأن الأسهم السعودية الصغيرة
   لها ATR منخفض فيكون RR دائماً قريب من 1.3 رغم احتمالية انفجار.
   لكن لا نقبل أقل من 1.3 - هذا الحد الأدنى المطلق.

⛔ الإقصاء 3 — أسبوعي هابط:
   إذا weekly_trend = "هابط":
     - مرفوض إلا بإجماع قوي (5+ إشارات تقنية active)
     - وحتى مع الإجماع، action لا يتعدى "مراقبة"
     - لا "شراء قوي" أبداً مع weekly هابط

⛔ الإقصاء 4 — Score منخفض:
   إذا score < 4.0، تجنّب الاختيار إلا لسبب استثنائي مذكور صراحة.

⛔ الإقصاء 5 — ADX ضعيف جداً:
   إذا ADX < 15 (سوق عرضي حقيقي)، لا تختر السهم لـ breakout strategy.

═══════════════════════════════════════════
✅ معايير التفضيل الإيجابية:
═══════════════════════════════════════════
- ADX > 25 (اتجاه قوي) → ميزة كبيرة
- MTF aligned (4 من 4) → ميزة كبيرة
- قطاع في "تسارع قوي" حسب تدفق السيولة → ميزة
...
═══════════════════════════════════════════
لكل سهم في picks، تأكد:
□ EV >= 0 (إلا "مراقبة")
□ R/R >= 1.5
□ weekly_trend ليس "هابط" (إلا بإجماع 5+)
□ score >= 4.0 (أو سبب استثنائي مكتوب)
□ confidence > 50% (إلا "مراقبة")

إذا أي بند لم يتحقق → احذف السهم من picks. أفضل 5 picks مفلترة من 10 picks ضعيفة.
  ...
  "picks": [
    {
      ...
      "confidence": 75,
      "reason": "سبب مركّز مستند إلى إشارات تقنية محددة + السياق القطاعي (لا تعتمد على ML)",
      "stop": 0,
      "target": 0,
      "target2": 0,
      "holding_days": 3,
